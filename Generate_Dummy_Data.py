...
ASSETS = [
    ...
    # ── Level 1 — Controllers ────────────────────────────────────────────
    {
        ...
        "recommended_actions": [
            "Enable DNP3 Secure Authentication v5",
            "Restrict DNP3 to known master IPs"
        ],
        "packet_count": 7241,
        "days_ago_first": 25
    },
    ...
    },
    {
        ...
        "recommended_actions": [
            "Disable RDP on jump server",
            "Enforce MFA on SSH sessions"
        ],
        "packet_count": 3102,
        "days_ago_first": 30
    },
    ...
    # ── Level 4 — Business / IT Network ─────────────────────────────────
    {
        ...
        "risk_level": "MEDIUM",
        "risk_reasons": [
            "Engineering workstation with both OT tool access and internet connectivity"
        ],
        "recommended_actions": [
            "Ensure workstation has EDR agent",
            "Restrict direct OT zone access from this host"
        ],
        "packet_count": 22410,
        "days_ago_first": 60
    },
...
def populate_db():
    # Always delete and recreate for a clean slate
    ...
    """)
    conn.commit()
    print(f"Created table in: {DB_FILE}\n")

    inserted = 0
    for a in ASSETS:
        try:
            first_seen, last_seen = generate_timestamps(a["days_ago_first"])
            llm_classification = json.dumps({
            ...
            conn.commit()  # commit every row individually
            inserted += 1
            print(f"  [{inserted:02d}] OK  {a['ip']:<18} {a['risk_level']:<8} {a['device_type']}")

        except Exception as e:
            print(f"  [!!] FAILED {a['ip']}: {e}")

    conn.close()

    print(f"\n{'='*55}")
    print(f"  Total inserted : {inserted} / {len(ASSETS)} assets")
    ...
    conn.close()
    print(f"\n  Verification: {count} rows confirmed in DB")

    if count == len(ASSETS):
        print("  All assets inserted successfully!")
    else:
        print("  WARNING: row count mismatch — check errors above")


if __name__ == "__main__":
    print("IT/OT Asset Dummy Data Generator")
    print("="*55)
    populate_db()
